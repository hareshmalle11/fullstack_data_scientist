...
st.set_page_config(
    page_title="Customer Segmentation (DBSCAN)",
    layout="wide"
)

BASE_DIR = os.path.dirname(os.path.abspath(__file__))

MODEL_PATH = os.path.join(BASE_DIR, "dbscan_model.pkl")
...
<style>
    ...
    .metric-value { color: #F43F5E; font-size: 32px; font-weight: 700; }
    .stButton>button {
        background: linear-gradient(135deg, #F43F5E 0%, #E11D48 100%);
        color: white; border: none; border-radius: 8px; padding: 12px 24px; font-weight: 700; width: 100%; transition: all 0.3s ease;
    }
    .stButton>button:hover { transform: translateY(-2px); box-shadow: 0 10px 15px -3px rgba(244, 63, 94, 0.4); }
</style>
""", unsafe_allow_html=True)

st.title("🌌 Density-Based Customer Clustering - DBSCAN")
st.write("Group customer profiles based on density using a DBSCAN Proxy Model.")
...
if st.button("🔮 Analyze & Segment Customer"):
    ...
    status_text = "Outlier / Noise (Unclassified)" if predicted_cluster == -1 else f"Core Cluster {predicted_cluster}"
    card_color = "#E11D48" if predicted_cluster == -1 else "#10B981"
    ...
    <div class="metric-card" style="border-left: 6px solid {card_color};">
        ...
        <p style="margin-top: 10px; color: #94A3B8;">
            Based on the credit card behavioral data provided, the DBSCAN proxy model classified this customer as <b>{status_text}</b>.
        </p>
    </div>
    """, unsafe_allow_html=True)
    
    if df_full is not None:
        st.subheader("📈 Core vs Outlier Visualizer")
        
        # Plot Purchases vs Balance
        plot_df = df_full.copy()
        plot_df['Group'] = plot_df['Cluster'].apply(lambda x: 'Outlier (-1)' if x == -1 else f'Cluster {x}')
        
        # Add the new user input point
        input_point = input_df.copy()
        input_point['Group'] = 'Input Customer'
        combined_df = pd.concat([plot_df, input_point], ignore_index=True)
        ...
        fig = px.scatter(
            ...
            y='PURCHASES', 
            color='Group',
            title='Customer Distribution: Balance vs Purchases',
            color_discrete_map={
                'Outlier (-1)': '#E11D48',
                'Input Customer': '#F59E0B',
                'Cluster 0': '#10B981',
                'Cluster 1': '#3B82F6',
                'Cluster 2': '#8B5CF6',
                'Cluster 3': '#EC4899'
