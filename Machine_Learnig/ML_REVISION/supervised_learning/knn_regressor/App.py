...
st.set_page_config(page_title="Insurance Charges Predictor (KNN)", layout="centered")
st.title("Insurance Charges Predictor - KNN")
st.write("Predict insurance charges from customer details.")

model = load_model()
if model is None:
    st.error(
        "Model file not found. Run knn_insurance_regression.ipynb to train and save knn_insurance_model.pkl."
    )
    st.stop()
